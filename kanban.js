...
!(function (t, e) {
  ...
  webix.protoUI(
    {
      ...
      defaults: {
        ...
        template: function () {
          var t =
            this.icon || (this.link ? "webix_kanban_icon kbi-plus-circle" : "");
          return (
            (t
              ? "<span class='webix_icon " +
                (this.link ? "webix_kanban_add_icon " : "") +
                t +
                "'></span>"
              : "") +
            "<span class='webix_strong' style='line-height:" +
            this.height +
            "px'>" +
            (this.label || "") +
            "</span>"
      ...
      _template_types: { sub: { css: "webix_kanban_sub_header" } },
      on_click: {
        webix_kanban_add_icon: function () {
          var t = { text: "" },
            e = this.getKanban(),
            i = e.queryView({ id: this.config.link });
          i &&
            this.callEvent("onBeforeCardAdd", [t, i]) &&
            (e.setListStatus(t, i), e.add(t));
        },
      },
      getKanban: function () {
  ...
  var e = { jpg: !0, jpeg: !0, png: !0, gif: !0 },
    i = {
      ppt: "-powerpoint",
      pptx: "-powerpoint",
      pptm: "-powerpoint",
  ...
  var t = {
    ...
    templateTags: function (t, e, i) {
      var n = [];
      if (t.tags)
        for (var a = i._tags, s = 0; s < t.tags.length; s++) {
          var o = t.tags[s];
          a.exists(o) && (o = a.getItem(o).value),
            n.push("<span class='webix_kanban_tag'>" + o + "</span>");
        }
      return (
        "<div  class='webix_kanban_tags'>" +
        (n.length ? n.join("") : "&nbsp;") +
        "</div>"
      );
    },
    templateIcons: function (t, e, i) {
      for (var n = [], a = null, s = "", o = 0; o < e.icons.length; o++)
        ((a = e.icons[o]).show && !a.show(t, i)) ||
          ((s =
            "<span webix_icon_id='" +
            (a.id || a.icon || a) +
            "' class='webix_kanban_footer_icon' title='" +
            (a.tooltip || "") +
            "'>"),
          (s += "<span class='" + (a.icon || a) + " webix_icon'></span>"),
          a.template &&
            (s +=
              "<span class='webix_kanban_icon_text'>" +
              webix.template(a.template)(t) +
              "</span>"),
          (s += "</span>"),
          n.push(s));
      return (
        "<div  class='webix_kanban_footer_icons'>" + n.join(" ") + "</div>"
      );
    },
    templateAvatar: function (t, e, i) {
      var n = i._users,
        a = t.user_id && n.exists(t.user_id) ? n.getItem(t.user_id) : {};
      return a.image
        ? "<img class='webix_kanban_avatar' src='" +
            a.image +
            "' title='" +
            (a.value || "") +
            "'>"
        : "<span class='webix_icon webix_kanban_icon kbi-account' title='" +
            (a.value || "") +
            "'></span>";
    },
    ...
    templateAttachments: function (t) {
      if (t.attachments)
        for (var e in t.attachments) {
          var i = t.attachments[e];
          if (n(i.link.split(".").pop()))
            return (
              "<img class='webix_kanban_attachment' src='" + i.link + "'/>"
            );
        }
      return "";
    },
    ...
    templateStart: webix.template(
      "<div webix_l_id='#id#' class='{common.classname()} webix_kanban_list_item' style='width:{common.width}px; height:{common.height}px;'>"
    ),
    template: function (t, e) {
      var i = webix.$$(e.master),
        n = i._colors.exists(t.color)
          ? i._colors.getItem(t.color).color
          : t.color,
        a =
        ...
        s =
          ...
          e.templateBody(t, e, i) +
          a +
          "</div>";
      return (
        "<div class='webix_kanban_list_content'" +
        (n ? " style='border-left-color:" + n + "'" : "") +
        ">" +
        (i.config.attachments ? e.templateAttachments(t, e, i) : "") +
        s +
        ("<div class='webix_kanban_footer'>" +
          e.templateFooter(t, e, i) +
          "</div>") +
        "</div>"
      );
    },
  ...
  (webix.KanbanView = {
    ...
    move: function (t, e, i, n) {
      if (((i = i || this), (n = n || {}), webix.isArray(t)))
        return webix.DataMove.move.call(this, t, e, i, n);
      var a = i !== this,
        s = this.getKanban(),
        o = s.getItem(t),
        r = webix.dp.$$(s.config.id);
      if (a) {
        if (!s.callEvent("onBeforeStatusChange", [t, i.config.status, i]))
          return;
        s.setListStatus(o, i),
          r
            ? r.ignore(function () {
      ...
      if (
        (webix.DataMove.move.call(i, t, e),
        this._fixOrder(),
        this !== i && i._fixOrder(),
        r)
      ) {
        var c = webix.copy(o);
        (c.webix_move_index = e),
          (c.webix_move_id = i.data.order[e + 1]),
          (c.webix_move_parent = i.config.serverStatus || i.config.status),
          r.save(t, "update", c);
      }
      return (
        a && s.callEvent("onAfterStatusChange", [t, i.config.status, i]), t
      );
    },
    ...
    _handle_icons: function (t, e, i) {
      var n = i.getAttribute("webix_icon_id"),
        a = this.type.icons;
      if (a)
        for (var s = 0; s < a.length; s++)
          "object" === o(a[s]) &&
            (a[s].id || a[s].icon) === n &&
            a[s].click &&
            a[s].click.call(this, e, t, i, this);
      "$avatar" === n
        ? this.getKanban().callEvent("onAvatarClick", [e, t, i, this])
        : this.getKanban().callEvent("onListIconClick", [n, e, t, i, this]);
    },
    $dragCreate: function (t, e) {
      ...
      return (
        (n.innerHTML = i),
        (n.className = "webix_drag_zone webix_kanban_drag_zone"),
    ...
    webix.protoUI(
      {
          ...
          this.files.data.scheme({
            $init: function (t) {
              "string" == typeof t.link &&
                t.link &&
                ((t.name = t.name || t.link.split("/").pop()),
                (t.type = t.type || t.name.split(".").pop()),
                (t.status = t.status || "server")),
                (t.sizetext = t.sizetext || i._format_size(t.size));
            },
          }),
        ...
        getValue: function () {
          var e = [];
          return (
            this.files.data.each(function (t) {
              "server" === t.status &&
                e.push({ id: t.id, link: t.link, size: t.size });
            }),
            e
          );
        },
    ...
    webix.type(webix.ui.dataview, {
      ...
      template: function (t, e) {
        return "<a"
          .concat(
            "server" === t.status
              ? ' href="'.concat(t.link, '" download="').concat(t.name, '"')
              : "",
            "></a>\n\t\t\t\t"
          )
          .concat(e.body(t), "\n\t\t\t\t")
      ...
      body: function (t) {
        return "server" === t.status && n(t.type)
          ? '<div class="webix_kanban_uploader_body"><img src="'.concat(
              t.link,
              '"></div>'
            )
          : "<div class='webix_kanban_uploader_body'>\n\t\t\t\t\t<span class='webix_icon webix_kanban_icon kbi-file".concat(
              (function e(t) {
      ...
      title: function (t, e) {
        return '<div class="webix_kanban_uploader_title" title="'
          .concat(t.name, '">\n\t\t\t\t\t')
          .concat(
            e.progress(t),
            '\n\t\t\t\t\t<div class="webix_kanban_uploader_label">'
          )
          .concat(t.name, "</div>\n\t\t\t\t</div>");
      },
      progress: function (t) {
        switch (t.status) {
          case "client":
            return (
              "<span class='webix_kanban_uploader_progress'>" +
              t.sizetext +
              "</span>"
            );
          case "transfer":
          ...
          case "server":
            return (
              "<span class='webix_kanban_uploader_progress_server'>" +
              t.sizetext +
              "</span>"
            );
          default:
  ...
  function s(t, e) {
    ...
    if ("object" === o(i)) for (var n in i) t[n] = i[n];
    else "function" == typeof i ? i.call(e, t, !0) : (t.status = i);
  }
  function r(e) {
    var i = e.config.status;
    return "object" === o(i)
    ...
    webix.protoUI(
      {
        ...
        $init: function (t) {
          ...
          webix.isArray(n)
            ? (a.elements = n)
            : "object" === o(n)
            ? (((a = webix.extend(a, n, !0)).view = "form"),
              (a.elements =
                a.elements || a.rows || (a.cols ? [{ cols: a.cols }] : s)),
              delete a.rows,
              delete a.cols)
            : (a.elements = s),
                  ...
                  cols: [
                    {
                      ...
                      click: function () {
                        var t = i.getValues({ hidden: !1 }),
                          e = i.getKanban();
                        e.callEvent("onBeforeEditorAction", ["remove", i, t]) &&
                          e._removeCard(t.id).then(function () {
                            return i._close();
                          });
                      },
                    },
                    {},
                    {
                      ...
                      click: function () {
                        var t = i.getValues({ hidden: !1 }),
                          e = i.getKanban();
                        e.callEvent("onBeforeEditorAction", ["save", i, t]) &&
                          (i._fixStatus(t, e),
                          e.exists(t.id)
                            ? ((t.$list = e.getItem(t.id).$list),
                              e.updateItem(t.id, t))
                            : e.add(t),
                          i._close());
                      },
        ...
        _afterInit: function () {
          ...
          if (t) {
            ...
            t.define("link", e.config.id),
              t.addDropZone(e.$view),
              webix.extend(e, webix.OverlayBox);
          }
          var i = this.queryView({ $hide: !0 }, "all");
          i.length &&
            this.attachEvent("onShow", function () {
              for (var t = 0; t < i.length; t++)
                i[t].getList().count() ? i[t].show() : i[t].hide();
            });
        },
        _fixStatus: function (t, e) {
          (t.$list = Number(t.$list) || 0),
            e._sublists[t.$list] && e.setListStatus(t, e._sublists[t.$list]);
        },
        getForm: function () {
          return this._form;
        },
        ...
        setValues: function (t) {
          ("object" === o(t) && t) || (t = {});
          var e = this.getKanban(),
            i = e._assignList(t);
          (t.$list = t.$list || (-1 !== i ? i : 0)),
            this._prepareEditor(e, t.id),
            this._form.setValues(t);
        },
    ...
    webix.protoUI(
      {
        ...
        defaults: {
          ...
          template: function (t) {
            return t.image
              ? "<img class='webix_kanban_list_avatar' src='" +
                  t.image +
                  "'>" +
                  t.value
        ...
        $init: function () {
          this.$ready.push(function () {
            this.attachEvent("onItemClick", function (t) {
              var e = this.getContext().id,
                i = this.getKanban();
              if (i.callEvent("onBeforeCardAction", [t, e]))
                switch (t) {
                  case "edit":
    ...
    webix.protoUI(
      {
          ...
          (t.padding = 0),
            this.$ready.push(function () {
              ...
              this.attachEvent("onHide", this._hide_chat),
                this.queryView({ view: "list" }).data.attachEvent(
                  "onStoreUpdated",
                  function (t, e, i) {
                    i && "paint" !== i && n._save();
                  }
        ...
        _save: function () {
          ...
          t &&
            i.exists(t.id) &&
            i.updateItem(t.id, { comments: e.serialize() });
        },
        _hide_chat: function () {
          var t = this.getBody();
          t.queryView({ view: "form" }).clear(),
            t.queryView({ view: "list" }).clearAll();
        },
        getKanban: function () {
          return webix.$$(this.config.masterId);
        },
    ...
    webix.protoUI(
      {
        ...
        $init: function (t) {
          ...
          (this.$view.className += " webix_kanban"),
            ...
            (this._tags = this._data_unification(t.tags)),
            (this._users = this._data_unification(t.users)),
            (this._colors = this._data_unification(t.colors)),
            this.$ready.push(function () {
              var n = this;
              this.reconstruct(),
                this._initEditor(),
                ...
                this._initMenu(),
                this._initComments(),
                this.data.attachEvent("onStoreUpdated", function (t, e, i) {
                ...
                this.attachEvent("onDestruct", function () {
                  for (var t = 0; t < n._destroy_with_me.length; t++)
                    n._destroy_with_me[t].destructor();
                });
            }),
        ...
        cardActions_setter: function (t) {
          if ((!0 === t && (t = ["edit", "copy", "remove"]), webix.isArray(t)))
        ...
        showEditor: function (t) {
          ...
          this.callEvent("onBeforeEditorShow", [e, t]) &&
            e &&
            (e.setValues(t),
            e.show(),
            this.callEvent("onAfterEditorShow", [e, t]));
        },
        copy: function (t) {
          if (this.callEvent("onBeforeCopy", [t])) {
            var e = webix.copy(this.getItem(t));
            delete e.id,
              (e.text = webix.i18n.kanban.copy + " " + (e.text || ""));
            var i = this.add(e),
              n = this.getOwnerList(i);
            n.move(i, n.getIndexById(t) + 1, n),
              this.callEvent("onAfterCopy", [t]);
          }
        },
        _removeCard: function (e) {
          var i = this,
            n = webix.promise.defer();
          return (
            webix.i18n.kanban.confirm
              ? webix.confirm({
        ...
        _data_unification: function (t) {
          ...
          return (
            this._destroy_with_me.push(e),
            t && "string" == typeof t ? e.load(t) : e.parse(t || []),
            e
          );
        },
        getEditor: function () {
        ...
        _initEditor: function () {
          ...
          if (this.config.editor) {
            ...
            (this._editor = t.config.id),
              this._destroy_with_me.push(t),
              this.attachEvent("onListIconClick", function (t, e) {
        ...
        _initUserList: function () {
          ...
          if (this.config.userList) {
            var t =
              "object" === o(this.config.userList) ? this.config.userList : {};
            webix.extend(
              t,
              {
                view: "kanbanuserlist",
              ...
              (t = webix.ui(t)),
              (this._userList = t.config.id),
              this._destroy_with_me.push(t),
              this.attachEvent("onAvatarClick", function (t, e, i) {
                var n = a.getUserList();
                n.setContext({ id: t, user_id: a.getItem(t).user_id }),
                  n.show(i);
              });
          }
        },
        _initMenu: function () {
          ...
          if (this.config.cardActions) {
            ...
            (this._menu = t.config.id),
              this._destroy_with_me.push(t),
              this.attachEvent("onListIconClick", function (t, e, i, n) {
                if ("menu" === t) {
                  var a = s.getMenu();
        ...
        _initComments: function () {
          ...
          if (this.config.comments) {
            var t =
              "object" === o(this.config.comments) ? this.config.comments : {};
            (t = webix.extend({ width: 400, height: 400 }, t, !0)),
              webix.extend(t, { view: "comments", users: this._users }, !0);
            var e = webix.ui({
              view: "kanbanchat",
              body: t,
              masterId: this.config.id,
            });
            (this._comments = e.config.id),
              this._destroy_with_me.push(e),
              this.attachEvent("onListIconClick", function (t, e, i, n) {
                if (
                  "comments" === t &&
                  c.callEvent("onBeforeCommentsShow", [e, i, n])
                ) {
                  var a = c.getComments(),
                    s = webix.html.offset(n);
                  (s.y += s.height), a.hide();
                  var o = a.getBody(),
                    r = c.getItem(e).comments || [];
                  o.parse(webix.copy(r)),
                    a.setContext({
                      id: e,
                    }),
                    a.show(s);
        ...
        _applyOrder: function (t, e, i) {
          ...
          if (-1 !== this.getIndexById(t))
            switch (i) {
              case "add":
                0 <= this._assignList(e) &&
                  (this.getOwnerList(t).add(e),
                  (e.$index = this._sublists[e.$list].count()));
                break;
              case "delete":
              ...
              case "update":
                var n = e.$list,
                  a = this._assignList(e);
                n === a
                  ? 0 <= a && this.getOwnerList(t).updateItem(t, e)
                  : (this._sublists[n] && this._sublists[n].remove(t),
                    0 <= a && this.getOwnerList(t).add(e));
            }
        },
        setListStatus: function (t, e) {
          for (var i = 0; i < this._sublists.length; i++)
            if (this._sublists[i] === e) return void s(t, e);
        },
        ...
        _prepareLists: function () {
          ...
          for (
            ...
            var n = e[i];
            "kanbanheader" !== n.config.view
              ? this._sublists.push(n)
              : (n.config.master = this.config.id);
          }
          for (var a = 0; a < this._sublists.length; a++) {
            var s = this._sublists[a];
            (this._subfilters[a] = r(s)),
              this.config.icons &&
                (s.type.icons = s.type.icons || this.config.icons);
            var o =
              s.config.name ||
              ("string" == typeof s.config.status && s.config.status
                ? s.config.status[0].toUpperCase() + s.config.status.slice(1)
                : s.config.id);
            t.push({ id: a.toString(), value: o }),
              (s.config.master = this.config.id),
              (s.type.master = this.config.id);
          }
        ...
        _syncData: function () {
          ...
          for (var e = 0; e < this._sublists.length; e++) {
            ...
            1 < n.length &&
              ...
              this._sublists[e].clearAll(),
              this._sublists[e].data.importData(n),
              this._sublists[e]._fixOrder();
          }
        },
        _assignList: function (t) {
          for (var e = 0; e < this._sublists.length; e++)
            if (this._subfilters[e](t)) return (t.$list = e);
          return -1;
        },
        ...
        eachList: function (t) {
          for (var e = 0; e < this._sublists.length; e++)
            t.call(this, this._sublists[e], this._sublists[e].config.status);
        },
      },
      webix.DataLoader,
      webix.EventSystem,
